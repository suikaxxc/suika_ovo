...
6. 浊度传感器使用 `GPIO01/ADC1` 读取AO模拟量；若模块由5V供电，AO必须经过分压后再接Hi3861 ADC（确保ADC端电压<=1.8V）
7. 浊度NTU已改为“多次采样平均 + 原始值自适应标定 + 平滑滤波”，可在不同5V浊度模块/分压方案下自动拉开清水与浑浊水差异（见 `turbidity_sensor.c`）
8. 建议上电后先在清水中稳定几秒，再放入浑浊水进行对比，以便自适应标定快速建立有效范围
