...
| 加热片 | GPIO10 | 加热控制 |
| PWM风扇 | GPIO04/PWM1 | 散热控制(PWM调速，默认按5V风扇驱动板“高电平有效”逻辑) |
| LED补光灯 | GPIO03 | 补光控制 |
...
5. 风扇电机电源可为5V（独立供电），代码只输出GPIO04/PWM1控制信号；默认按“高电平有效”的5V风扇驱动板逻辑控制
6. AZDM01浊度传感器使用 `GPIO01/ADC1` 读取AO模拟量；模块5V供电时，AO常见范围约0~4.5V，必须先分压再接Hi3861 ADC（确保ADC端电压<=1.8V）。推荐分压比约1:3（如上臂20kΩ、下臂10kΩ），4.5V输入时ADC端约1.5V
7. 浊度NTU已改为“多次采样平均 + 清水基线自适应 + 浑浊端自适应 + 平滑滤波”；上电初始值会优先贴近清水区（约20~30）而非居中约500（见 `turbidity_sensor.c`）
8. 建议上电后先在清水中稳定几秒，再放入浑浊水/遮挡光路，算法会将浑浊端快速拉高到高NTU区间（常见可到700+）
