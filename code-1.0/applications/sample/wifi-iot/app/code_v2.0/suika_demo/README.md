...
5. AZDM01为0.5V~4.5V模拟输出，接入Hi3861 ADC前需使用分压电路（推荐分压比2.5，如 R1=15kΩ 上拉、R2=10kΩ 下拉）；其中公式中的 `V_out` 指传感器原始输出电压（分压前电压），软件按 `NTU = -125 * V_out + 625` 计算并在NTU>300时报警
...
1. **分压比与代码不一致**  
   当前代码按分压比2.5重建 `V_out`（示例：R1=15kΩ, R2=10kΩ）。若实物分压比不同，会导致换算后的 `V_out` 固定在错误区间。
