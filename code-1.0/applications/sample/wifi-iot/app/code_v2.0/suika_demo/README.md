...
- 使用DS18B20测量水温
- 水温过低时启动加热片加热
- 水温超过设定上限时风扇以100%全速散热，未超过上限时关闭风扇
- 将水温维持在最适区间
...
| 加热片 | GPIO10 | 加热控制 |
| PWM风扇 | GPIO04/PWM1 | 散热控制(PWM调速，0%时会强制切GPIO高电平确保风扇关闭) |
| LED补光灯 | GPIO03 | 补光控制 |
...
5. AZDM01为0.5V~4.5V模拟输出，接入Hi3861 ADC前需使用分压电路（推荐分压比2.5，如 R1=15kΩ 上端电阻、R2=10kΩ 下端电阻）；当前代码参考MQ2采样思路，采用GPIO01/ADC1直接多次采样平均后再计算浊度
6. AZDM01供电应为5V（VCC接5V、GND接地、AO接GPIO01）；当前代码按5V供电配置并取消了启动预热延时
...
1. **分压比与代码不一致**  
   当前代码仍会重建 `vout` 仅用于诊断日志（示例：分压比2.5，R1=15kΩ, R2=10kΩ）。若实物分压比不同，日志电压会异常，且极端情况下raw动态范围会被压缩；由于现在使用固定电压到NTU映射，分压偏差会直接带来NTU偏差，因此分压比必须与代码一致。

2. **AO接线或地线问题**  
   AO未正确接入GPIO01/ADC1、GND未共地、接触不良时，ADC容易读到近似固定中间值。

3. **供电或接线不稳定**  
   当前代码不再做启动预热，若VCC/GND/AO接触不良或5V电源波动，读数可能表现为固定值或抖动异常。

4. **供电电压与预期不一致**  
   本代码按5V供电语义处理；若模块实际只有3.3V或电源波动，清水电压会显著低于预期，NTU会偏高并可能看起来“固定”。

5. **ADC饱和（raw长期接近0或4095）**  
   若分压网络不匹配（例如满量程仍超过Hi3861 ADC 1.8V输入范围），raw会长期贴近极值并导致NTU几乎不变。当前代码已增加连续饱和告警：  
